...
def main():
    parser = argparse.ArgumentParser(description='Display mobilenet v3 mini classification results.')
    ...
    parser.add_argument('-l', '--labels_file',
                        help='Path to imagenet_1000_labels.txt', required=True)
    ...
    if len(labels) != 1001:
        raise RuntimeError('Invalid labels_file: need 1001 categories')
    ...
    if len(input_files) <= 0:
        ...
        for idx, val in enumerate(input_files):
            ...
            print(cur_results_dir)
            cur_results_file = os.path.join(output_dir, cur_results_dir, 'MobilenetV3/Predictions/Softmax.raw')
